...
parser.add_argument("--git", help="git repo with ostro metadata")
parser.add_argument("--workdir", default='/workdir',
                    help="Directory containing the ostro metadata. "
                         "Or the location to prepare the git metadata"
                         "if --git was specfied.")
...
try:
    ...
    ostrofound = os.path.exists(os.path.join(args.workdir,".git"))

    if ostrofound and args.git:
    ...
    elif not ostrofound and not args.git:
        errormsg = ('An ostro git repository was not found in {}. "--git"'
                    'must be specified.')
        errormsg = errormsg.format(args.workdir)
        raise OstroLaunchError(errormsg)

    elif not ostrofound and args.git:
        clone_ostro(args.git, args.workdir)
